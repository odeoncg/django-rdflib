...
class SQL(Store):
    ...
    def __init__(self, identifier=None, configuration=None,
        ...
        self._internedId = INTERNED_PREFIX + hashlib.sha1(self.identifier).hexdigest()[:10]
    ...
    def executeSQL(self,cursor,qStr,params=None,paramList=False):
        ...
        #pprint((qStr, params, paramList))
        #if params:
            #if paramList:
                #for item in param:
                    #pprint(qStr % item)
            #else:
                #pprint(qStr % [str(p) for p in params])
        #else:
            #pprint(qStr)
    ...
    def destroy(self, configuration=None):
        ...
        for tbl in self.tables + ["%s_namespace_binds"%self._internedId]:
            try:
                self.executeSQL(c, 'DROP TABLE %s CASCADE' % (tbl,))
    ...
    def empty(self):
        ...
        for tbl in self.tables + ["%s_namespace_binds"%self._internedId]:
            try:
                self.executeSQL(c, 'TRUNCATE TABLE %s CASCADE' % (tbl,))
                #print "dropped table: %s"%(tblsuffix%(self._internedId))
            except Exception, e:
                print >> sys.stderr, "unable to truncate table: %s"%(tbl)
                print >> sys.stderr, e
    ...
    def remove(self, (subject, predicate, obj), context):
        ...
        for brp in targetBRPs:
            ...
            #print query+whereClause, whereParameters
    ...
    def removeN(self, quads):
        ...
        for subject, predicate, obj, context in quads:
            #pprint((subject, predicate, obj, context))
            targetBRPs = BinaryRelationPartitionCoverage((subject,predicate,obj,context),self.partitions)
            for brp in targetBRPs:
                #pprint(brp)
                query = "DELETE from %s WHERE "%(
                ...
                queries[brp] = query
                quadWhereClause, quadWhereParameters = brp.generateWhereClause((subject,predicate,obj,context))
                if not where_clauses.has_key(brp):
                    where_clauses[brp] = []
                where_clauses[brp].append(quadWhereClause)
                if not where_params.has_key(brp):
                    where_params[brp] = []
                where_params[brp] += quadWhereParameters
        #pprint(queries)
        #pprint(where_clauses)
        #pprint(where_params)
        for brp in queries.keys():
            query = queries[brp]
            whereClause = '(%s)' % ') OR ('.join(where_clauses[brp])
            whereParameters = where_params[brp]
            #print query+whereClause, whereParameters
            self.executeSQL(c,query+whereClause,params=whereParameters)
    ...
    def namespaces(self):
        """ """
        c=self._db.cursor()
        self.executeSQL(c,"select prefix, uri from %s_namespace_binds;"%(
            self._internedId
            )
        )
        rt=c.fetchall()
